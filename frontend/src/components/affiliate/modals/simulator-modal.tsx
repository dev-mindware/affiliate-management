...
export function SimulatorModal() {
  ...
  // 1. Determine Level
  const currentLvl = [...levels]
    .reverse()
    .find((lvl) => clients >= lvl.minClients) || levels[0];

  // 2. Base + Level Bonus
  const baseRate = currentLvl.base;
  const levelBonus = currentLvl.bonus;

  // 3. Retention/Quality Modifier (churn)
  ...
  if (churn < 2) {
    ...
    retentionModifier = -levelBonus; // suspend level bonus
    retentionLabel = "Churn Elevado (Bónus de Nível Suspenso)";
  }

  // 4. Ramp-up by client antiquity (average antiquity <= 12 months)
  ...
  if (nextLvl) {
    ...
    // Additional gain is monthly commission at next level minus current
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-5">
          ...
          {/* Modificadores info */}
          <div className="text-xs space-y-1.5 p-3 rounded-lg border border-border/80 bg-card">
            <div className="flex justify-between">
              <span className="text-muted-foreground">Status Qualidade (Churn):</span>
              <span className="font-semibold text-foreground">{retentionLabel}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Ramp-Up (Antiguidade):</span>
              <span className="font-semibold text-foreground">{rampUpLabel}</span>
            </div>
          </div>
          ...
          {nextLvl ? (
            <div className="space-y-2 pt-1">
              ...
              <div className="bg-primary/5 dark:bg-primary/10 rounded-lg p-3 border border-primary/20 mt-2">
            ...
            <div className="bg-purple-50 dark:bg-purple-950/30 rounded-lg p-3 border border-purple-200 dark:border-purple-900/50">
